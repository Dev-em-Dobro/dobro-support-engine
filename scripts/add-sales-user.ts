/**
 * add-sales-user — bootstrap / rotação de credenciais do Agente de Vendas.
 *
 * Cada vendedor tem credencial própria armazenada em sales_users com hash scrypt.
 *
 * Uso:
 *   npm run sales:add -- --email=vendedor@devemdobro.com --password='senhaForte123'
 *
 *   # Desativar (sem deletar):
 *   npm run sales:add -- --email=vendedor@devemdobro.com --deactivate
 *
 *   # Reativar:
 *   npm run sales:add -- --email=vendedor@devemdobro.com --activate
 *
  ...
  try {
    if (args.deactivate || args.activate) {
      ...
      const r = await pool.query(
        `UPDATE sales_users SET active = $1 WHERE email = $2 RETURNING email, active`,
        [active, args.email]
      );
      if (r.rowCount === 0) {
        console.error(`Vendedor não encontrado: ${args.email}`);
        process.exit(1);
      }
    ...
    const r = await pool.query(
      `INSERT INTO sales_users (email, password_hash)
         VALUES ($1, $2)
    ...
    const row = r.rows[0];
    console.log(`✔ Vendedor cadastrado/atualizado: ${row.email}`);
